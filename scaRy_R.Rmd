...
This post is reflection of my journey learning `R` as a statistical programming language. I will use **`r version[['version.string']]`**. I intend it to be read by seasoned `R` users (to remind them of what they should not overlook when training juniors) as well as new `R` users (to tell them what should not discourage them) whom have not even downloaded the program.
...
This can be too much to expect from newbie. What it does is that it helps to install packages that need compilation. An analogy to this is taking a Masters degree programme in Germany. Now, German students can be admitted without any language course requirements. However, if you come from non-German speaking country, you may need a 'compiler' to train you on speaking German language then you get enrolled into your course of choice. Now some packages, forget about what packages are now, may need compilation before they are usable in `R` and that is the role of `RTools`. However, as a new user, most packages you will need can run without compilation: They speak same language as `R`! So, keep going!
...
You will be trained how to drive at a certain place. However, once you master the rules of driving, the trainer will not follow you throughout your life to show you how to drive everywhere you go. In fact, you can train others. In that same spirit, once you are introduced into `R`, keep exploring and teaching others. Today you are being trained, tomorrow you will train others. Do not feel left alone. It is the best way to learn `R`. Your current trainer was also trained not long ago.
