...
Learning `R`, to the best of my knowledge, like most other programming languages (though I am yet to gather adequate experience in those `other` programming languages), is normally received by learners  with _zero_ background in programming with awe. Why? Because the training normally comes from seasoned users, who have used the program for a long time that they know, but overlook, basics that a new user should grasp while making their initial step into `R`. 
