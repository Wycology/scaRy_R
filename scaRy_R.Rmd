...
This post is reflection of my journey learning `R` as a statistical programming language. I will use **`r version[['version.string']]`**. I intend it to be read by seasoned `R` users (to remind them of what they should not overlook when traing juniors) as well as new `R` users (to tell them what should not discourage them) whom have not even downloaded the program.
